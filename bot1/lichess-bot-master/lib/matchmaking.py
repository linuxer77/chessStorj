...
class Matchmaking:
    ...
    def __init__(self, li: Lichess, config: Configuration, user_profile: UserProfileType) -> None:
        ...
        self.last_challenge_created_delay = Timer(seconds(50))  # Challenges expire after 20 seconds.
        self.last_game_ended_delay = Timer(seconds(1))
        self.last_user_profile_update_time = Timer(minutes(5))
        self.min_wait_time = seconds(10)  # Wait before new challenge to avoid api rate limits.
    ...
    def choose_opponent(self) -> tuple[str | None, int, int, int, str, str]:
        ...
        target_bot = match_config.get("target_bot")
        if target_bot:
            logger.info(f"Directly challenging target bot: {target_bot}")
            return target_bot, base_time, increment, days, variant, mode
    ...
    def challenge(self, active_games: set[str], challenge_queue: MULTIPROCESSING_LIST_TYPE, max_games: int) -> None:
        ...
        self.challenge_target = bot_username if challenge_id else ""
