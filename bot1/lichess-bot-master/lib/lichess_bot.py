    ...
    with multiprocessing.pool.Pool(max_games + 1) as pool:
        while not (stop.terminated or (one_game and one_game_completed) or stop.restart):
            ...
            check_in_on_correspondence_games(pool,
                                             ...
                                             active_games,
                                             max_games)
            accept_challenges(li, challenge_queue, active_games, max_games, matchmaker)
...
def accept_challenges(li: lichess.Lichess, challenge_queue: MULTIPROCESSING_LIST_TYPE, active_games: set[str],
                      max_games: int, matchmaker: matchmaking.Matchmaking) -> None:
    """Accept a challenge."""
    while len(active_games) < max_games and challenge_queue:
        ...
        # Check for collision with our own outgoing challenge
        if matchmaker.challenge_id:
            if matchmaker.challenge_target == chlng.challenger.name:
                logger.info(f"Incoming challenge from target {chlng.challenger.name}. Cancelling our challenge {matchmaker.challenge_id} and accepting theirs.")
                try:
                    li.cancel(matchmaker.challenge_id)
                except Exception:
                    pass
                matchmaker.discard_challenge(matchmaker.challenge_id)
            else:
                logger.info(f"Ignoring challenge from {chlng.challenger.name} because we have a pending challenge to {matchmaker.challenge_target}.")
                # Put it back or discard? If we discard, they might retry. If we put back, we might loop.
                # Discarding is safer to avoid blocking the queue with ignored challenges.
                continue
